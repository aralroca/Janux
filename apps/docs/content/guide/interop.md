...
2. **Reactive props bridge.** `options.props(own)` receives the JSX call-site props (typically the enclosing island's `state`) and is **tracked**: signal reads re-render only the foreign root — no Janux DOM diffing crosses the boundary. The host element is an opaque leaf for the morph: on SPA navigations the diff skips a live root's children entirely (React's fibers keep node references, and DOM rewritten behind their back means bailed-out renders and `removeChild` crashes on unmount) — the refreshed call-site props are pushed into the live root instead, which re-renders itself.
