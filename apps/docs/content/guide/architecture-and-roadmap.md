...
- **Platform:** arbitrary HTTP handlers (`src/api/**`, full grammar, method exports → `Response`) and multipart uploads (`request.formData()` + client `dropzone`) ship. **Streaming SSR & component-property Suspense** are RFC-specified but not yet implemented — async data reaches the UI reactively through the client cache today; streaming SSR is a first-paint optimization tracked with the compiler.
- **Agentic surface v1:** the hosted MCP endpoint (`/_janux/mcp`, streamable HTTP, stateless, optional bearer auth with `WWW-Authenticate`), per-page **Markdown projections** (`.md` suffix + `janux://page/*` MCP resources) and **proposal visual diffs** (confirm proposals carry a shadow-run before/after of the component state for pure intents) all ship. `llms.txt` and Web Bot Auth were already in v0.
