...
Everything the RFC's two archetypes need is implemented and tested:
...
- **Routing:** parallel routes (`@slot`) and intercepting routes (`(.)`) — URL-addressable modals are covered today by query-string state; named-slot routing lands only if a migration needs it.
