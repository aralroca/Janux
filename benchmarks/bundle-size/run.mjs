...
// Two app sets: js-framework rows and TodoMVC. TodoMVC's ops use a distinct
// prefix so every set rides one suite with one baseline file.
const SETS = [
	{
		root: JS_FRAMEWORK,
		prefix: '',
			...
			build: {
				...
				rollupOptions: {
					output: {
						...
						// the Janux workspace runtime — Bun resolves it via the node_modules
						// symlink to packages/janux — and `\0` virtuals) so the index.html entry
							...
							groups: [
								{
									...
									test: (id) => {
										...
										return (
											...
											clean.includes(`${path.sep}packages${path.sep}janux`)
										);
									},
								},
							],
