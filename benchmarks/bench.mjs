// Unified Janux benchmark runner — the CI/regression layer that makes every
// per-suite number load-bearing.
//
// Derived from octane/benchmarks/bench.mjs (MIT) — Copyright (c) 2026 Dominic
// Gannaway. See benchmarks/LICENSE-OCTANE and CREDITS.md. Adapted for the Janux
// monorepo: Bun workspaces instead of pnpm, and a suite manifest trimmed to the
// multi-framework suites Janux participates in.
//
...
//   node benchmarks/bench.mjs --quick js-framework todomvc
...
const SUITES = [
	...
	},
	{
		...
		// target and times the built SSR + hydration (for Janux: resume from
		// serialized snapshots). One invocation per target; the per-target
		// single-target payloads are merged into one `news` result.
		name: 'news',
		cwd: 'news',
		servers: [],
		iter: { normal: 20, quick: 3 },
		runs: perTarget(FRAMEWORKS),
	},
	{
		// Real pre-hydration typing against a withheld production client chunk,
		// measured with Chromium CDP CPU throttling. The target apps reuse news's
		// existing SSR toolchains.
	...
	},
	{
		...
		// total, chunk framing + all-fast throughput. Svelte is N/A because its
		// public renderer is buffered. Iteration-counted (renders per target per
		// scenario).
		name: 'streaming-ssr',
		cwd: 'streaming-ssr',
		servers: [],
...
// Start `bun run --filter <filter> preview` detached, logging to the results
// dir. The corresponding `build` has already run, so browser suites compare
