...
The playground's agent panel gained a second checkbox for the cursor (on by default, like the glow's), and every example that boots `glow: true` now boots `cursor: true` alongside it.
