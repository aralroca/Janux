...
export function htmlDocument(options: ShellOptions): string {
  ...
  return [
    '<!doctype html>',
    '<html>',
    `<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${safeAttr(options.title ?? 'Janux app')}</title>${description}${favicon}${manifestLink}${styleLinks}</head>`,
    '<body>',
    options.html,
