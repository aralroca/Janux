...
export const Counter = component({
  ...
  view: ({ state, intents }: any) => (
    <div class="counter">
      ...
      <button on={intents.add}>+1</button>
      <button on={intents.setTo} data-input={JSON.stringify({ value: 0 })}>
        reset
      </button>
      <button on={intents.setTo.with({ value: 10 })}>ten</button>
      <form intent={intents.save} reset>
        <input name="note" />
      </form>
    </div>
  ),
