 ...
 */
export function createHandler(root = process.cwd()): { fetch(request: Request): Promise<Response> } {
...
async function boot(root: string): Promise<{ fetch(request: Request): Promise<Response> }> {
  return createJanuxServer(await prodServerOptions(root));
}
