...
const APP = 'examples/with-advanced-routing';
...
describe('examples/with-advanced-routing end to end', () => {
  it('renders a wiki article per [slug] param, title included', async () => {
    ...
    expect(await (await get('/wiki/definitely-not-written')).text()).toContain('Not written yet');
  });

  it('resolves deep catch-all routes and exposes the joined segments', async () => {
    const html = await (await get('/docs/guides/deploy/vercel')).text();
    ...
    expect(await (await get('/docs/guides')).text()).toContain('<span class="segment-count">1</span>');
  ...
  it('serves the optional catch-all with and without a rest', async () => {
    const bare = await (await get('/search')).text();
    const filtered = await (await get('/search/kind/article')).text();
  ...
  it('gates /tickets by typed matchers: digits and uuids match, anything else 404s', async () => {
    const byNumber = await get('/tickets/123');
    const byUuid = await get(`/tickets/${UUID}`);
    ...
    expect((await get('/tickets/abc')).status).toBe(404);
    expect((await get('/tickets/12.5')).status).toBe(404);
    expect((await get('/tickets/2b0d7b3d-8b8f-4a1e-9d3a')).status).toBe(404);
  });

  it('wraps each section in its own layout chain and no other', async () => {
    const wiki = await (await get('/wiki/islands')).text();
