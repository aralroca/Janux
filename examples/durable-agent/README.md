...
- **Guardrails** — `unicodeNormalizer` plus an `injectionGuard` classifier screen every turn; hostile input is aborted with a typed refusal and the app answers with its own safe reply, the model never sees it.
...
| `src/server/guardrails.ts` | The processor chain, the injection classifier and the safe refusal the UI answers with |
