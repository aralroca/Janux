...
- **Validation as part of the contract** — `evals/validation.eval.json` asserts that malformed input answers a structured `400` naming the offending field, even on the `confirm`-guarded tool.
...
| `evals/write-off.eval.json` | The approval flow: proposal → untouched state → approve → new state → consumed proposal |
| `evals/validation.eval.json` | Structured `400`s for bad input, including on the guarded tool |
