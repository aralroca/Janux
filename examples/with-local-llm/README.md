...
- **`probeLocalLlm()` picks the default** — a real `requestAdapter()` probe on mount (headless browsers expose `navigator.gpu` with no usable adapter, so `'gpu' in navigator` alone lies); browsers that fail it land on `serverLlm()`, the built-in `/_janux/llm` mount (resolved from `JANUX_MODEL` or a provider key).
...
The e2e suite covers the *mechanics* (probe-based detection, fallback, toggle, consent gate, degradation) and even a full local turn — a scripted provider injected through `localLlm({ provider })` stands in for the GPU model, so the tool call lands on the task list in headless CI. Only the actual inference needs hardware:
...
| File | What it shows |
|---|---|
| `src/copilot.ts` | The runtime: the `probeLocalLlm()` gate, one cached `localLlm()` session, `serverLlm()` (whose setup-card refusals read the same in stream and non-stream mode), and the brain swap that disposes the old loop |
