...
- **Zero wiring for the feedback** — `createCopilot({ visualize })` is the whole of it. The framework feeds it from `janux:tool-call` (intents) and `janux:tool-target` (the DOM fallback), and stands the built-in `glow` down while it runs.
