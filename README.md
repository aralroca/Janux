...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-3747%20passing-brightgreen" alt="3747 tests passing" />
...
- [`examples/interop-drag-drop`](examples/interop-drag-drop) — `@dnd-kit` unchanged: a drag whose event is an unserializable object graph becomes a typed `move` intent, which the agent calls without dragging anything.
- [`examples/interop-command-palette`](examples/interop-command-palette) — `cmdk` unchanged: the palette's command list and the agent's tool enum are asserted to be the same list.
- [`examples/interop-forms`](examples/interop-forms) — `react-hook-form` + `zod` unchanged: the honest two-owner case, where an agent's fill has to be reconciled into uncontrolled inputs.
- [`examples/interop-graph-editor`](examples/interop-graph-editor) — `@xyflow/react` unchanged, in both directions: a node drag and a drawn edge become `moveNode` / `connect`, the same tools the agent calls.
- [`examples/interop-a11y-primitives`](examples/interop-a11y-primitives) — `@radix-ui/react-dialog` unchanged: portals escape the host, and a navigation with the dialog open neither throws nor leaves the scroll locked.
...
- [`examples/with-sass`](examples/with-sass) — Sass with no config beyond the file extension: `src/styles.scss` with tokens, nesting, a mixin and four accent classes generated by one `@each` loop, all compiled to a single `/styles.css`.
- [`examples/with-css-variables`](examples/with-css-variables) — runtime theming: island state writes `--brand`/`--pad`/`--radius` onto one wrapper and the cascade rethemes the page, with no rebuild and no extra stylesheet.
- [`examples/with-worker`](examples/with-worker) — `worker()`: the same prime-counting function on a Web Worker and on the main thread, with a ticker that proves which one froze the page.
...
- [`examples/with-advanced-routing`](examples/with-advanced-routing) — the full router grammar: `[slug]`, `[...path]`, `[[...rest]]`, `[id=integer]`/`[uid=uuid]` matchers, nested `_layout.tsx` chains, `(marketing)` groups and the `_404.tsx`/`_500.tsx` pages, plus SPA navigation with a `persist` island.
